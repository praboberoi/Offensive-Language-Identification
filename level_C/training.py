...
class OffensiveLanguageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        return torch.tensor(self.X[idx].toarray().squeeze(), dtype=torch.float32), torch.tensor(self.y[idx], dtype=torch.long)

def load_process_file(file, seperator, header):
    data = pd.read_csv(file, sep=seperator, header=header)
    data = data.dropna()
    data["class_label"] = np.argmax(data[["average_ind", "average_grp", "average_oth"]].values.tolist(), axis=1)
    return data
...
def test_model(test_loader, device, model):
    ...
    f1 = f1_score(all_labels, all_predections, average='weighted')
...
def save_bag_of_words_mode(vectorizer):
    with open("./logistic_regression/level_C/count_vectorizer.pkl", "wb") as f:
        pickle.dump(vectorizer, f)

def sampling_test(vectorizer, model, device):
    class_names = ['ind', 'grp', 'oth']
    ...
    while sample_text != 'q':
        ...
        sample_output = model(sample_tensor).squeeze(1)
        print(f'sample_output : {sample_output}') 
        _, predicted_class = torch.max(sample_output, dim=1)
        predicted_class = predicted_class.item()
        print(f'predicted_class : {predicted_class}')
        print(f"The sample text is predicted to be class {class_names[predicted_class]}.")

        sample_text = input('please enter a text for test or q to quit: ')


def main():
    data = load_process_file('./formated_file_c.tsv', seperator="\t", header=0)
    ...
    torch.save(model.state_dict(), "./logistic_regression/level_C/logistic_regression_model.pt")
