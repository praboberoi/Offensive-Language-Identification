...
def save_bag_of_words_mode(vectorizer):
    with open("./logistic_regression/level_B/count_vectorizer.pkl", "wb") as f:
...
def sampling_test(vectorizer, model, device):
    ...
    while(sample_text != 'q'):
        ...
        if sample_output > 0.5:
            print("The sample text is predicted to be untargeted.")
        else:
            print("The sample text is predicted targited.")

        sample_text = input('please enter a text for test or q to quit :    ')

def main():
    data = load_process_file('./formated_file_b.tsv', seperator="\t", header=0)
    ...
    torch.save(model.state_dict(), "./logistic_regression/level_B/logistic_regression_model.pt")
